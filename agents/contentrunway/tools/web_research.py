...
class WebResearchTool:
    ...
    def __init__(self, searxng_base_url: str = "http://localhost:8080/search"):
        ...
        # Domain-specific sources (updated with working feeds)
        self.domain_sources = {
            "it_insurance": [
            ...
            "ai": [
                # Updated to working feeds - removed broken ones
    ...
    async def _search_rss_feed(
        ...
        try:
            ...
            # Ensure query is a string and handle list cases
            ...
            for entry in feed.entries[:20]:  # Check first 20 entries
                ...
                try:
                    relevance_score = sum(1 for term in query_terms if term in content) / len(query_terms) if query_terms else 0.5
                except Exception as e:
    ...
    async def search_trending_topics(
        ...
        NOTE: External platform search is disabled due to access restrictions.
        
        Args:
            base_topic: Base research topic
            domain: Target domain
            platforms: Social platforms to search (currently disabled)
            max_results: Maximum number of trending results to return
            
        Returns:
            Empty list - external trending search disabled
        """
        logger.info(f"Trending search for '{base_topic}' in domain '{domain}' - external platforms disabled")
        
        # Return empty list since external access is blocked
        logger.warning("External platform search disabled due to access restrictions")
        return []
