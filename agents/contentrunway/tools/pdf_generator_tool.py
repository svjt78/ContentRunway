...
class PDFGeneratorTool:
    ...
    async def generate_pdf_from_content(
        ...
        if not content or len(content.strip()) < 100:
            error_msg = f"PDF generation failed: Content too short ({len(content)} chars). Content preview: '{content[:100]}'"
            self.logger.log_error("generate_pdf", error_msg)
            raise ValueError(error_msg)
        
        if not title or title.strip() == "":
            error_msg = f"PDF generation failed: Title is empty or invalid: '{title}'"
            self.logger.log_error("generate_pdf", error_msg)
            raise ValueError(error_msg)
        
        self.logger.log_info("generate_pdf", f"🔧 DEBUG: Starting PDF generation - Title: '{title}', Content: {len(content)} chars")
        self.logger.log_info("generate_pdf", f"🔧 DEBUG: Content preview: {content[:200]}")
        ...
        try:
            ...
            # CRITICAL: Validate PDF was actually generated
            if len(pdf_data) < 1000:  # A real PDF should be at least 1KB
                error_msg = f"PDF generation failed: Generated file too small ({len(pdf_data)} bytes). This indicates PDF creation failed."
                self.logger.log_error("generate_pdf", error_msg)
                raise Exception(error_msg)
            
            self.logger.log_info("generate_pdf", f"🔧 DEBUG: PDF generated successfully - Size: {len(pdf_data)} bytes")
            ...
            import traceback
            self.logger.log_error("generate_pdf", f"🔧 DEBUG: Full traceback: {traceback.format_exc()}")
            self.logger.log_error("generate_pdf", f"🔧 DEBUG: Error type: {type(e)}")
            self.logger.log_error("generate_pdf", f"🔧 DEBUG: Error message: {str(e)}")
