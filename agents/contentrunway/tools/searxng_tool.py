...
class SearXNGTool:
    ...
    def __init__(self, searxng_base_url: str = "http://localhost:8080"):
    ...
    async def _execute_searxng_search(
        ...
        """Execute search query against SearXNG using POST method."""
        
        # SearXNG search parameters for POST request
        data = {
            'q': query,
            'format': 'json',
            'categories': 'general',
        ...
        try:
            ...
            if 'application/json' not in content_type:
                logger.warning(f"SearXNG returned non-JSON response for query '{query}': {content_type}")
                return []
