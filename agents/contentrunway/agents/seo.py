...
from ..tools.content_tools import CONTENT_SEO_TOOLS, optimize_seo, analyze_content_quality
...
class SEOStrategistAgent:
    ...
    def __init__(self, model_name: str = "gpt-4o-mini", enable_tool_selection: bool = True):
        ...
        if enable_tool_selection:
            self.llm = self.base_llm.bind_tools(CONTENT_SEO_TOOLS)
        else:
    ...
    async def execute(
        ...
        logger.info(f"Developing SEO strategy for: {get_title(topic)}")
        ...
        try:
            ...
            target_keywords = get_target_keywords(topic)
            return {
                'outline': fallback_outline,
                'keyword_strategy': {'primary': target_keywords[0] if target_keywords else get_title(topic)},
                'error': str(e)
            }
    ...
    async def _develop_keyword_strategy(
        ...
        Current keywords: {', '.join(get_target_keywords(topic))}
        ...
        try:
            ...
            return {
                'primary_keyword': target_keywords[0] if target_keywords else title.lower(),
                'secondary_keywords': target_keywords[1:4] if len(target_keywords) > 1 else [],
                'long_tail_keywords': [f"{title.lower()} guide", f"how to {title.lower()}"],
    ...
    async def _create_content_outline(
        ...
        for source in sources[:15]:
            ...
            if keyword_matches > 0 or any(kw.lower() in source_text for kw in get_target_keywords(topic)):
        ...
        try:
            ...
            try:
                ...
                if not outline_data.get('sections') or len(outline_data.get('sections', [])) == 0:
                ...
                outline = Outline(
                    ...
                    target_audience=outline_data.get('target_audience', f'Professionals in {get_domain(topic)}'),
                    primary_angle=outline_data.get('primary_angle', get_description(topic)),
                ...
                logger.warning(f"JSON parsing failed: {json_error}, using fallback outline")
    ...
    def _create_fallback_outline(self, topic: TopicIdea, state: ContentPipelineState) -> Outline:
        ...
        logger.info(f"Creating fallback outline for topic: {get_title(topic)}")
        ...
        try:
            ...
            # Create outline with safe access to topic attributes
            primary_keyword = 'ai development'
            secondary_keywords = ['artificial intelligence', 'machine learning', 'technology trends']
            
            target_keywords = get_target_keywords(topic)
            if target_keywords:
                primary_keyword = target_keywords[0]
                if len(target_keywords) > 1:
                    secondary_keywords = target_keywords[1:4]
            
            outline = Outline(
                sections=sections,
                estimated_word_count=target_word_count,
                target_audience=f'Professionals in {get_domain(topic)}',
                primary_angle=get_description(topic) or f'Comprehensive guide to {get_title(topic)}',
                key_takeaways=['Expert insights', 'Practical guidance', 'Industry trends'],
                primary_keyword=primary_keyword,
                secondary_keywords=secondary_keywords
            )
            
            logger.info(f"Created fallback outline with {len(sections)} sections")
            return outline
            
        except Exception as e:
            logger.error(f"Fallback outline creation failed: {e}")
            # Ultra-simple fallback
            simple_sections = [
                {'title': 'Introduction', 'estimated_words': 300, 'key_points': ['Overview']},
                {'title': 'Main Content', 'estimated_words': 800, 'key_points': ['Key insights']},
                {'title': 'Conclusion', 'estimated_words': 200, 'key_points': ['Summary']}
            ]
            ...
            return Outline(
                ...
                primary_angle=f'Guide to {get_title(topic)}',
                key_takeaways=['Key insights'],
                primary_keyword='guide',
                secondary_keywords=['information', 'insights']
            )
    ...
    async def _generate_seo_recommendations(
        ...
        try:
            ...
            return {
                'optimized_title': get_title(topic)[:60],
                'meta_description': get_description(topic)[:160],
