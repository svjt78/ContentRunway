...
class ContentFormatterAgent:
    ...
    def __init__(self, model_name: str = "gpt-4o-mini"):
        ...
        self.platform_guidelines = {
            ...
            'digitaldossier': {
                'max_word_count': 5000,
                'ideal_word_count': 2500,
                'heading_style': 'title_case',
                'paragraph_length': 'professional',  # Professional document structure
                'supports_code_blocks': True,
                'supports_images': False,  # Handled separately as cover images
                'supports_lists': True,
                'formatting_style': 'professional_markdown',
                'seo_focus': 'medium',
                'document_focus': 'high'  # Focus on document readability for PDF
            }
        }
    ...
    async def execute(
        ...
        try:
            ...
            # Fallback: return original content in basic format for both platforms
            fallback_content = {
                'title': draft.title,
                'content': draft.content,
                'summary': draft.abstract or 'Professional document content',
                'meta_description': f"Document: {draft.title}",
            ...
            fallback_drafts = ChannelDrafts(
                personal_blog=fallback_content,
                digitaldossier=fallback_content
            )
            return {
                'channel_drafts': fallback_drafts,
                'platforms_formatted': ['personal_blog', 'digitaldossier'],
                'error': str(e)
            }
    ...
    def _determine_target_platforms(self, state: ContentPipelineState) -> List[str]:
        ...
        # Default platforms for Phase 1 - INCLUDE DIGITALDOSSIER
        return ['personal_blog', 'digitaldossier']
    ...
    async def _format_for_platform(
        ...
        if platform == 'digitaldossier':
            return await self._format_for_digitaldossier(draft, platform, guidelines, state)
        else:
            # Default to blog formatting for other platforms
    ...
    async def _format_for_digitaldossier(
        self,
        draft: Draft,
        platform: str,
        guidelines: Dict[str, Any],
        ...
        """Format content specifically for DigitalDossier document publishing."""
        
        logger.info(f"Formatting content for DigitalDossier platform")
        
        try:
            # Create optimized content for PDF document format
            formatted_content = {
                'title': draft.title,
                'content': draft.content,  # Use full content for documents
                'summary': draft.abstract or self._generate_summary_from_content(draft.content),
                'meta_description': draft.meta_description or self._generate_meta_description(draft.title, draft.content),
                'keywords': draft.keywords or [],
                'tags': draft.tags or [],
                'word_count': draft.word_count,
                'platform': platform,
                'document_type': 'academic_paper' if 'research' in draft.title.lower() else 'professional_document',
                'formatting_style': guidelines['formatting_style'],
                'abstract': draft.abstract or self._generate_summary_from_content(draft.content),
                'citations': getattr(draft, 'citations', []) or [],
                'reading_time_minutes': getattr(draft, 'reading_time_minutes', 0) or max(1, draft.word_count // 200),
                'readability_score': getattr(draft, 'readability_score', None)
            }
            
            # Ensure content is properly formatted for PDF generation
            if not formatted_content['content']:
                raise ValueError("No content available for DigitalDossier formatting")
            
            # Content should already be well-formatted from writing stage
            # Just ensure it meets document standards
            content_length = len(formatted_content['content'])
            if content_length < 500:
                logger.warning(f"Content might be too short for professional document: {content_length} characters")
            
            formatted_content['processing_notes'] = {
                'original_word_count': draft.word_count,
                'final_word_count': len(formatted_content['content'].split()),
                'platform_optimizations': [
                    'Professional document structure maintained',
                    'Academic formatting preserved',
                    'Citations and references included',
                    'Abstract/summary generated'
                ]
            }
            
            logger.info(f"DigitalDossier formatting completed: {formatted_content['word_count']} words")
            return formatted_content
            
        except Exception as e:
            logger.error(f"DigitalDossier formatting failed: {e}")
            # Fallback: return basic document format
            return {
                'title': draft.title,
                'content': draft.content,
                'summary': draft.abstract or "Professional document content",
                'meta_description': f"Document: {draft.title}",
                'keywords': draft.keywords or [],
                'tags': draft.tags or ['document'],
                'word_count': draft.word_count,
                'platform': platform,
                'formatting_notes': f'Fallback formatting due to error: {str(e)}'
            }
    
    def _generate_summary_from_content(self, content: str, max_length: int = 300) -> str:
        """Generate a summary from content if no abstract exists."""
        if not content:
            return "Professional document content"
        
        # Extract first meaningful paragraph as summary
        paragraphs = content.split('\n\n')
        for para in paragraphs:
            clean_para = para.strip().replace('#', '').strip()
            if len(clean_para) > 50:  # Skip very short lines
                if len(clean_para) <= max_length:
                    return clean_para
                else:
                    return clean_para[:max_length-3] + "..."
    ...
    def _generate_meta_description(self, title: str, content: str, max_length: int = 160) -> str:
        """Generate meta description for document."""
        if not title:
            return "Professional document"
        
        # Try to create informative meta description
        summary = self._generate_summary_from_content(content, 120)
        if summary and summary != "Professional document content":
        ...
        return f"Read {title} - Professional document with comprehensive analysis and insights."
