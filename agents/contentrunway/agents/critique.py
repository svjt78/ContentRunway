...
class CritiqueAgent:
    ...
    def __init__(self, model_name: str = None):
        # Use optimized model configuration for comprehensive critique
        config = get_optimized_model_config("CritiqueAgent", task_type="final_review")
...
Evaluate if these specific issues have been addressed."""
        ...
        if quality_scores is None:
            ...
            domain_score = quality_scores.domain_expertise or 0.0
            style_score = quality_scores.style_consistency or 0.0
            compliance_score = quality_scores.compliance or 0.0
            technical_score = quality_scores.technical_depth or 0.0
...
- Overall: {overall_score:.2f} (threshold: 0.85)
- Fact Check: {fact_check_score:.2f}
- Domain Expertise: {domain_score:.2f}
- Style Consistency: {style_score:.2f}
- Compliance: {compliance_score:.2f}
- Technical Depth: {technical_score:.2f}
...
{{
    ...
    "publication_readiness": {{
        ...
        try:
            ...
            try:
                critique_analysis = json.loads(content)
            except json.JSONDecodeError as json_error:
                logger.warning(f"Critique analysis JSON parsing failed: {json_error}")
                # Try to find JSON within the content
                import re
                json_match = re.search(r'\\{.*\\}', content, re.DOTALL)
                if json_match:
                    json_str = json_match.group(0)
                    critique_analysis = json.loads(json_str)
                    logger.info("Successfully extracted critique analysis JSON from response")
                else:
                    logger.error("No JSON object found in critique analysis response")
                    raise json_error
...
{{
    ...
    "success_metrics": [
        ...
        try:
            ...
            # Try to extract JSON if there's extra text
            try:
                targeted_feedback = json.loads(content)
            except json.JSONDecodeError as json_error:
                logger.warning(f"Targeted feedback JSON parsing failed: {json_error}")
                # Try to find JSON within the content
                import re
                json_match = re.search(r'\\{.*\\}', content, re.DOTALL)
                if json_match:
                    json_str = json_match.group(0)
                    targeted_feedback = json.loads(json_str)
                    logger.info("Successfully extracted targeted feedback JSON from response")
                else:
                    logger.error("No JSON object found in targeted feedback response")
                    raise json_error
