...
class TitleGeneratorAgent:
    ...
    def __init__(self):
        ...
        self.agent_name = "TitleGeneratorAgent"
        # Get optimized config for classification tasks
        self.model_config = get_optimized_model_config("TitleGeneratorAgent", task_type="classification")
...
{{
    ...
    async def execute(
        ...
        if not content_dict or not isinstance(content_dict, dict):
            error_msg = f"Title generation failed: Invalid content_dict: {type(content_dict)}"
            self.logger.log_error("title_generation", error_msg)
            raise ValueError(error_msg)
        
        content = content_dict.get('content', content_dict.get('body', ''))
        if not content or len(content.strip()) < 100:
            error_msg = f"Title generation failed: Content too short ({len(content)} chars). Content: '{content[:100]}'"
            self.logger.log_error("title_generation", error_msg)
            raise ValueError(error_msg)
        
        original_title = content_dict.get('title', 'Untitled')
        if not original_title or original_title.strip() == "":
            error_msg = f"Title generation failed: Invalid original title: '{original_title}'"
            self.logger.log_error("title_generation", error_msg)
            raise ValueError(error_msg)
        
        self.logger.log_info("title_generation", f"🔧 DEBUG: Starting title generation - Original: '{original_title}', Classification: '{classification}'")
        self.logger.log_info("title_generation", f"🔧 DEBUG: Content length: {len(content)} chars, Preview: {content[:150]}")
        ...
        try:
            ...
            import traceback
            self.logger.log_error("title_generation", f"🔧 DEBUG: Full traceback: {traceback.format_exc()}")
            self.logger.log_error("title_generation", f"🔧 DEBUG: Error type: {type(e)}")
            self.logger.log_error("title_generation", f"🔧 DEBUG: Error message: {str(e)}")
            self.logger.log_error("title_generation", f"🔧 DEBUG: Content dict keys: {list(content_dict.keys()) if content_dict else 'None'}")
            self.logger.log_error("title_generation", f"🔧 DEBUG: Original title: '{content_dict.get('title', 'NO_TITLE')}'") 
            
            # Check if it's an API key issue
            if "api_key" in str(e).lower() or "unauthorized" in str(e).lower():
                self.logger.log_error("title_generation", "🔧 CRITICAL: OpenAI API key missing or invalid!")
            ...
            self.logger.log_warning("title_generation", f"🔧 DEBUG: Using fallback title: '{fallback_title}'")
